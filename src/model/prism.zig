...
fn zeroSlice(slice: []f32) void { for (slice) |*v| v.* = 0.0; }
fn normalizeInPlace(vec: []f32) void {
    var norm_sq: f32 = 0.0;
    for (vec) |v| norm_sq += v * v;
    if (norm_sq <= 1.0e-12) return;
...
pub const PrismLayer = struct {
    ...
    pub fn init(allocator: std.mem.Allocator, config: config_mod.PrismConfig, hidden_dim: usize, head_dim: usize, device: tensor_mod.Device, device_id: i32, rng: *std.Random) !*PrismLayer {
        ...
        errdefer { var i: usize = 0; while (i < beta_initialized) : (i += 1) w_beta[i].deinit(); }
        errdefer { var i: usize = 0; while (i < k_initialized) : (i += 1) w_k[i].deinit(); }
        errdefer { var i: usize = 0; while (i < p_initialized) : (i += 1) w_p[i].deinit(); }
        ...
        self.* = .{ .config = config, .hidden_dim = hidden_dim, .num_iterations = num_iterations, .head_dim = head_dim, .shortconv = shortconv, .w_beta = w_beta, .w_k = w_k, .w_p = w_p, .alpha = config.forget_factor, .allocator = allocator, .device = device, .device_id = device_id };
        return self;
    }
    ...
    fn validateForwardInputs(self: *PrismLayer, input: *Tensor, v: *Tensor, state: ?*PrismState) !void {
        ...
        if (state) |s| {
            if (s.state.shape.ndim != 3) return error.InvalidStateShape;
            if (s.state_dim != self.head_dim or s.value_dim != self.head_dim) return error.InvalidStateShape;
            if (s.state.shape.dim(0) != input.shape.dim(0) or s.state.shape.dim(1) != self.head_dim or s.state.shape.dim(2) != self.head_dim) return error.InvalidStateShape;
        }
    }
    ...
    pub fn forward(self: *PrismLayer, input: *Tensor, v: *Tensor, state: ?*PrismState) !struct { output: *Tensor, new_state: *PrismState } {
        ...
        defer u.deinit();
        var new_state = try PrismState.init(self.allocator, batch_size, self.head_dim, self.head_dim, self.device, self.device_id);
        ...
        if (self.device == .cuda) try self.prismForwardCuda(u, v, state, new_state, output) else try self.prismForwardCpu(u, v, state, new_state, output);
        return .{ .output = output, .new_state = new_state };
    }

    fn prismForwardCuda(self: *PrismLayer, u: *Tensor, v: *Tensor, prev_state: ?*PrismState, new_state: *PrismState, output: *Tensor) !void {
        ...
        try kernels.prismForwardCuda(u.ptr(), v.ptr(), if (prev_state) |s| s.state.ptr() else null, new_state.state.ptr(), output.ptr(), w_beta_ptrs.ptr, w_k_ptrs.ptr, w_p_ptrs.ptr, u.shape.dim(0), u.shape.dim(1), self.hidden_dim, self.head_dim, self.num_iterations, self.alpha);
    }

    fn prismForwardCpu(self: *PrismLayer, u: *Tensor, v: *Tensor, prev_state: ?*PrismState, new_state: *PrismState, output: *Tensor) !void {
        ...
        for (0..batch_size) |b| {
            ...
            if (prev_state_ptr) |ps| {
                for (0..self.head_dim) |row| for (0..self.head_dim) |col| current_state[row * self.head_dim + col] = ps[b * self.head_dim * self.head_dim + row * self.head_dim + col].toFloat32();
            }
            for (0..seq_len) |t| {
                const token_offset = (b * seq_len + t) * self.hidden_dim;
                zeroSlice(refined);
                for (0..self.head_dim) |d| residual[d] = v_ptr[token_offset + d].toFloat32() - u_ptr[token_offset + d].toFloat32();
                for (0..self.num_iterations) |l| {
                    ...
                    for (0..self.head_dim) |d| p_vec[d] = sigmoid(p_vec[d]);
                    for (0..self.head_dim) |d| delta[d] = geluApprox(p_vec[d] * residual[d]);
                    if (l == 0) {
                        for (0..self.head_dim) |row| {
                            var sum: f32 = 0.0;
                            for (0..self.head_dim) |col| sum += current_state[row * self.head_dim + col] * key_vec[col];
                            state_times_key[row] = sum;
                        }
                        for (0..self.head_dim) |row| for (0..self.head_dim) |col| next_state[row * self.head_dim + col] = self.alpha * current_state[row * self.head_dim + col] - self.alpha * beta_scalar * state_times_key[row] * key_vec[col] + beta_scalar * delta[row] * key_vec[col];
                    } else {
                        for (0..self.head_dim) |row| for (0..self.head_dim) |col| next_state[row * self.head_dim + col] = current_state[row * self.head_dim + col] + beta_scalar * delta[row] * key_vec[col];
                    }
                ...
                for (0..self.head_dim) |d| out_ptr[token_offset + d] = dtype_mod.BF16.fromFloat32(u_ptr[token_offset + d].toFloat32() + refined[d]);
            }
            for (0..self.head_dim) |row| for (0..self.head_dim) |col| new_state_ptr[b * self.head_dim * self.head_dim + row * self.head_dim + col] = dtype_mod.BF16.fromFloat32(current_state[row * self.head_dim + col]);
        }
    }
    ...
    pub fn backward(self: *PrismLayer, grad_output: *Tensor, input: *Tensor, v: *Tensor, state: *PrismState) !struct { grad_input: *Tensor, grad_v: *Tensor, grad_state: *PrismState } {
        try self.validateForwardInputs(input, v, state);
        if (self.device == .cuda) {
            var grad_output_cpu = try grad_output.to(self.allocator, .cpu, 0);
            defer grad_output_cpu.deinit();
            var input_cpu = try input.to(self.allocator, .cpu, 0);
            defer input_cpu.deinit();
            var v_cpu = try v.to(self.allocator, .cpu, 0);
            defer v_cpu.deinit();
            var state_cpu = try PrismState.init(self.allocator, state.state.shape.dim(0), state.state_dim, state.value_dim, .cpu, 0);
            defer state_cpu.deinit();
            if (@hasDecl(Tensor, "copyFrom")) try state_cpu.state.copyFrom(state.state) else return error.UnsupportedDevice;
            const cpu_result = try self.backwardCpu(grad_output_cpu, input_cpu, v_cpu, state_cpu);
            defer cpu_result.grad_input.deinit();
            defer cpu_result.grad_v.deinit();
            defer cpu_result.grad_state.deinit();
            var grad_input = try cpu_result.grad_input.to(self.allocator, self.device, self.device_id);
            errdefer grad_input.deinit();
            var grad_v_dev = try cpu_result.grad_v.to(self.allocator, self.device, self.device_id);
            errdefer grad_v_dev.deinit();
            var grad_state_dev = try PrismState.init(self.allocator, state.state.shape.dim(0), state.state_dim, state.value_dim, self.device, self.device_id);
            errdefer grad_state_dev.deinit();
            if (@hasDecl(Tensor, "copyFrom")) try grad_state_dev.state.copyFrom(cpu_result.grad_state.state) else return error.UnsupportedDevice;
    ...
    fn backwardCpu(self: *PrismLayer, grad_output: *Tensor, input: *Tensor, v: *Tensor, state: *PrismState) !struct { grad_input: *Tensor, grad_v: *Tensor, grad_state: *PrismState } {
        ...
        var grad_state = try PrismState.init(self.allocator, state.state.shape.dim(0), self.head_dim, self.head_dim, self.device, self.device_id);
        ...
        for (0..state.state.shape.dim(0) * self.head_dim * self.head_dim) |i| grad_state_ptr[i] = dtype_mod.BF16.fromFloat32(0.0);
        ...
        defer for (0..self.num_iterations) |l| { grad_w_p[l].deinit(); grad_w_beta[l].deinit(); grad_w_k[l].deinit(); };
        for (0..batch_size) |b| {
            for (0..seq_len) |t| {
                const token_offset = (b * seq_len + t) * self.hidden_dim;
                for (0..self.hidden_dim) |d| token_grad_u[d] = grad_output_ptr[token_offset + d].toFloat32();
                for (0..self.head_dim) |d| residual[d] = v_ptr[token_offset + d].toFloat32() - u_ptr[token_offset + d].toFloat32();
                for (0..self.num_iterations) |l| {
                    projectTokenInto(p_vec, self.w_p[l], u_ptr, token_offset, self.hidden_dim, self.head_dim);
                    for (0..self.head_dim) |d| {
                ...
                while (l_rev > 0) {
                    l_rev -= 1;
                    const grad_w_p_ptr = grad_w_p[l_rev].typedPtr(dtype_mod.BF16).?;
                    for (0..self.head_dim) |d| {
                    ...
                    for (0..self.hidden_dim) |i| for (0..self.head_dim) |d| {
                        const idx = i * self.head_dim + d;
                        grad_w_p_ptr[idx] = dtype_mod.BF16.fromFloat32(grad_w_p_ptr[idx].toFloat32() + u_ptr[token_offset + i].toFloat32() * grad_linear[d]);
                    }
                    accumulateMatVecTransposeToInput(token_grad_u, self.w_p[l_rev], grad_linear, self.hidden_dim, self.head_dim);
                    for (0..self.head_dim) |d| grad_residual_next[d] = grad_residual_before[d];
                }
                for (0..self.head_dim) |d| {
                    token_grad_u[d] -= grad_residual_next[d];
                    grad_v_ptr[token_offset + d] = dtype_mod.BF16.fromFloat32(grad_v_ptr[token_offset + d].toFloat32() + grad_residual_next[d]);
                }
                for (0..self.hidden_dim) |d| grad_u[token_offset + d] = token_grad_u[d];
            }
        }
        for (0..batch_size) |b| for (0..seq_len) |t| for (0..self.hidden_dim) |d| {
            var sum: f32 = 0.0;
            for (0..self.shortconv.window_size) |w| {
                const out_t = t + w;
                if (out_t < seq_len) sum += grad_u[(b * seq_len + out_t) * self.hidden_dim + d] * weight_ptr[d * self.shortconv.window_size + w].toFloat32();
            }
        ...
        const grad_sc_ptr = grad_shortconv_weight.typedPtr(dtype_mod.BF16).?;
        const input_ptr = input.typedPtr(dtype_mod.BF16).?;
        for (0..batch_size) |b| for (0..seq_len) |t| for (0..self.hidden_dim) |d| for (0..self.shortconv.window_size) |w| {
            const lookback = @as(isize, @intCast(t)) - @as(isize, @intCast(w));
            if (lookback >= 0) {
                const src_t = @as(usize, @intCast(lookback));
                const grad_u_val = grad_u[(b * seq_len + t) * self.hidden_dim + d];
...
pub const ShortConv = struct {
    ...
    pub fn forward(self: *ShortConv, input: *Tensor) !*Tensor {
        ...
        if (self.device == .cuda) {
            ...
            try kernels.shortConvForwardCuda(input.ptr(), self.weight.ptr(), output.ptr(), batch_size, seq_len, self.hidden_dim, self.window_size);
        } else {
...
pub const PrismState = struct {
    ...
    pub fn init(allocator: std.mem.Allocator, num_states: usize, state_dim: usize, value_dim: usize, device: tensor_mod.Device, device_id: i32) !*PrismState {
        ...
        self.* = .{ .state = state_tensor, .state_dim = state_dim, .value_dim = value_dim, .allocator = allocator };
        return self;
    }
